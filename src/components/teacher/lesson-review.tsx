...
export function LessonReview({ lessonId }: { lessonId: string }) {
  ...
  const draftClassId = typeof draft?.class_id === "string" ? draft.class_id : "";
  const selectedClassIds = classIds ?? (draftClassId ? [draftClassId] : []);
  ...
  const publishDisabled = publish.isPending || !approved || rejected.size > 0 || (!followUp && (selectedClassIds.length === 0 || !title.trim()));
    ...
    <section className="lesson-immersive draft-review-page">
      <header className="draft-review-actionbar">
        ...
        <div>
          ...
          {!approved && <button className="secondary-button" onClick={() => approve.mutate()} disabled={approve.isPending || rejected.size > 0}>{approve.isPending ? <CircleNotch className="animate-spin" size={16} /> : <Check size={16} />} Duyệt toàn bộ bản nháp</button>}
