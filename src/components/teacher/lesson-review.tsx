...
export function LessonReview({ lessonId }: { lessonId: string }) {
  ...
  const pdfEligible = Boolean(matrix && matrix.slots.length === 12 && poolDeficitCount === 0 && rejected.size === 0);
    ...
    <section className="lesson-immersive draft-review-page">
      <header className="draft-review-actionbar">
        ...
        <div>
          ...
          {!followUp && <button className="secondary-button" disabled={reviewMutationPending || pdfGenerating || !pdfEligible || pdfArtifactQuery.isLoading} onClick={handlePdfAction} title={!pdfEligible ? "Cần đủ 12 bài Mastery và không còn câu Cần thay" : undefined}>{pdfGenerating ? <CircleNotch className="animate-spin" size={16} /> : pdfArtifact?.status === "READY" ? <ArrowSquareOut size={16} /> : <FilePdf size={16} />} {pdfGenerating ? "Đang tạo" : pdfArtifact?.status === "READY" ? "Bản học sinh" : pdfArtifact?.status === "FAILED" ? "Thử lại bản HS" : "Xuất bản HS"}</button>}
          {!followUp && <button className="secondary-button" disabled={reviewMutationPending || teacherPdfGenerating || !pdfEligible || teacherPdfArtifactQuery.isLoading} onClick={handleTeacherPdfAction} title={!pdfEligible ? "Cần đủ 12 bài Mastery và không còn câu Cần thay" : undefined}>{teacherPdfGenerating ? <CircleNotch className="animate-spin" size={16} /> : teacherPdfArtifact?.status === "READY" ? <ArrowSquareOut size={16} /> : <FilePdf size={16} />} {teacherPdfGenerating ? "Đang tạo" : teacherPdfArtifact?.status === "READY" ? "Bản giáo viên" : teacherPdfArtifact?.status === "FAILED" ? "Thử lại bản GV" : "Xuất bản GV"}</button>}
