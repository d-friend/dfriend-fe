...
import { downloadLessonReportExcel } from "@/lib/lesson-report-excel";
...
function LessonDetail({ lesson, studentCount, onSelectReport }: { lesson: ClassLessonItem | undefined; studentCount: number; onSelectReport: (id: string) => void }) {
  ...
  const reviewHref = Number.isInteger(lesson.taxonomyVersion) && Number(lesson.taxonomyVersion) > 0
    ? `/teacher/lessons/${encodeURIComponent(lesson.lessonId)}/review?taxonomyVersion=${lesson.taxonomyVersion}`
    : null;
  const pdfArtifacts = [
...
function ReportDetailInner({ summary, classId, completedCount, studentCount }: { summary?: CopilotReportSummary; classId: string; completedCount: number; studentCount: number }) {
  ...
  const exportReport = useMutation({
    mutationFn: async () => {
      if (!summary?.publicationId) throw new Error("Báo cáo chưa gắn với publication.");
      return teacherApi.lessonReportExport(classId, summary.publicationId);
    },
    onSuccess: downloadLessonReportExcel,
  });
...
function formatPercent(value: number | null | undefined) { return typeof value === "number" ? `${Math.round(value * 100)}%` : "Chưa đánh giá"; }
