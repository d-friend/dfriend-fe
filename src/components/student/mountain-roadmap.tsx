...
import { ArrowRight, Check, Flag, LockKey, MapTrifold, Path, Sparkle } from "@phosphor-icons/react";
          ...
          {roadmapQuery.data.map((item, index) => {
            ...
            return (
              <motion.div key={item.id} className="trail-stage" data-status={item.status} data-side={index % 2 ? "right" : "left"} initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                ...
                {item.extra_exercises?.length ? <Link className="trail-branch" href={item.extra_completed ? `/student/report/${item.extra_exercises[0].publication_id || `extra_${item.lessonId}`}` : `/student/lesson/${item.extra_exercises[0].publication_id || `extra_${item.lessonId}`}/part1`}><Flag size={16} /> {item.extra_completed ? "Feedback luyện thêm" : "Luyện thêm"}</Link> : null}
              </motion.div>
            );
          })}
