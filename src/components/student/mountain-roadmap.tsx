          ...
          {roadmapQuery.data.map((item, index) => {
            ...
            return (
              <motion.div key={item.id} className="trail-stage" data-status={item.status} data-side={index % 2 ? "right" : "left"} initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                ...
                {item.extra_exercises?.length ? <Link className="trail-branch" href={`/student/lesson/extra_${item.lessonId}/part2`}><Flag size={16} /> Luyện thêm</Link> : null}
              </motion.div>
            );
          })}
...
export function ProfileWorkspace() {
  ...
  const emptyMetrics = !metrics || (!metrics.thinking_score && !metrics.skill_score && !metrics.result_score);
  ...
  return <div className="student-page profile-page"><section className="profile-identity"><span className="profile-avatar">{fullName.split(/\s+/).slice(-2).map((p) => p[0]).join("").toUpperCase()}</span><div><p>Hồ sơ học tập</p><h1>{fullName}</h1><span>{classesQuery.data?.length || 0} lớp đang tham gia</span></div></section><section className="profile-grid"><div className="profile-competence"><div className="student-section-heading"><div><h2>Năng lực của bạn</h2><p>Thang 0-10, cập nhật sau mỗi Session 2.</p></div></div>{metricsQuery.isError ? <div className="student-empty-inline">Chưa tải được dữ liệu năng lực.</div> : emptyMetrics ? <div className="competence-empty-large"><MapTrifold size={34} /><strong>Chưa đủ dữ liệu để vẽ năng lực</strong><span>Hoàn thành Session 2 đầu tiên để nhận phản hồi.</span></div> : <div className="competence-numbers"><Score label="Tư duy" value={metrics?.thinking_score || 0} /><Score label="Kỹ năng" value={metrics?.skill_score || 0} /><Score label="Kết quả" value={metrics?.result_score || 0} /></div>}</div><div className="profile-history"><div className="student-section-heading"><div><h2>Bài đã hoàn thành</h2><p>Mở lại phản hồi theo từng bài học.</p></div><span>{completed.length} bài</span></div>{completed.length ? completed.map((item) => <Link key={`${item.className}-${item.id}`} href={`/student/report/${item.lessonId || item.id}`}><span><strong>{item.title}</strong><small>{item.className}</small></span><ArrowRight size={17} /></Link>) : <div className="student-empty-inline">Các bài hoàn thành sẽ xuất hiện ở đây.</div>}</div></section></div>;
}
