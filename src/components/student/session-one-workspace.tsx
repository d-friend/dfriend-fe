...
function Checkpoint({ question, id, answer, attempted, onAnswer, onAttempt }: { question: LessonOneQuestion; id: string; answer: string; attempted: boolean; onAnswer: (value: string) => void; onAttempt: () => void }) {
  ...
  const isTrueFalse = (question.type || question.exercise_type) === "true_false";
  const sourceOptions = isTrueFalse && !question.options?.length
    ? [{ label: "true", text: "Đúng" }, { label: "false", text: "Sai" }]
    : question.options || [];
  const options = sourceOptions.map((option, index) => typeof option === "string" ? { label: String.fromCharCode(65 + index), text: option } : { label: option.label || String.fromCharCode(65 + index), text: option.text || "" });
  ...
  return <form className="checkpoint-card" onSubmit={submit}><MathContent className="checkpoint-question">{question.questionText || question.question || "Câu hỏi kiểm tra"}</MathContent>{options.length ? <div className="checkpoint-options" data-question-type={isTrueFalse ? "true_false" : undefined}>{options.map((option) => <label key={option.label} data-selected={answer === option.label}><input type="radio" name={id} value={option.label} checked={answer === option.label} onChange={(event) => onAnswer(event.target.value)} disabled={attempted} />{!isTrueFalse && <span>{option.label}</span>}<MathContent className="checkpoint-option" answer>{option.text}</MathContent></label>)}</div> : <div className="student-field"><label htmlFor={id}>Câu trả lời của bạn</label><input id={id} value={answer} onChange={(event) => onAnswer(event.target.value)} disabled={attempted} /></div>}{localError && <p className="student-form-error" role="alert">{localError}</p>}{attempted ? <div className={correct ? "checkpoint-feedback correct" : "checkpoint-feedback retry"}>{correct ? <CheckCircle size={21} weight="fill" /> : <XCircle size={21} weight="fill" />}<div><strong>{correct ? "Chính xác" : "Chưa đúng, nhưng bạn đã thử"}</strong><MathContent className="checkpoint-explanation">{question.explanation || (correct ? "Bạn đã nắm phần này." : "Đọc lại lời giải thích rồi tiếp tục khi sẵn sàng.")}</MathContent></div></div> : <button className="student-secondary-button" type="submit">Kiểm tra</button>}</form>;
}
